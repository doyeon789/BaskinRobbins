...
public class CT_FrameWork {
    ...
    public static void Panel(JPanel panel_change, JFrame frame) {
        ...
        Select_Done.addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e){
                ...
                if(isSelect_done){
                    ...
                    JPanel panel5 = OrderDone_Main.getPanel5();
        ...
        AppleMint = new JLabel("");
        AppleMint.setVisible(false);
        AppleMint.setBounds(144, 678, 70,70);
        ImageIcon RainbowSI = new ImageIcon("Img/CheckAgain/ChangTaste/AppleMint.png");
        Image RainbowS_img = RainbowSI.getImage();
        Image RainbowS_logo = RainbowS_img.getScaledInstance(70,70, Image.SCALE_SMOOTH);
        AppleMint.setIcon(new ImageIcon(RainbowS_logo));

        panel_change.add(Select_Done);
        panel_change.add(Select_Done_Gray);

        panel_change.add(Recommend);
        panel_change.add(All);
        panel_change.add(Fruit);
        panel_change.add(Chocolate);
        panel_change.add(MilkCheese);
        panel_change.add(CoffeCaramelTea);
        panel_change.add(VanillaMint);
        panel_change.add(GrainNuts);
        panel_change.add(Selected);

        panel_change.add(IscrollPane);
        panel_change.add(AppleMint);

        panel_change.add(PST);
        panel_change.add(OrderLabel);
        panel_change.add(ToStart);
        panel_change.add(Background);
    }
}
